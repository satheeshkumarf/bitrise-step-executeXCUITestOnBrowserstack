...
test_execution_input="{\"devices\": [\"${device_name}\"], \"app\": \"$browserstack_app_url\", \"only-testing\" : [\"${test_name}\"], \"deviceLogs\" : \"true\", \"local\" : \"$browserstack_local\", \"localIdentifier\" : \"$browserstack_local_id\", \"testSuite\": \"$browserstack_xcuitest_url\", \"customBuildName\" : \"${BUILD_IDENTIFIER}\" , \"setEnvVariables\" : {\"$browserstack_env_var_name\":\"$browserstack_env_var_value\"}}"
